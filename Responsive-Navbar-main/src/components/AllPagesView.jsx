...
const PageCard = React.forwardRef(
    ({ page, layoutKey, updateContent, deleteItem, setActivePageId }, ref) => {
        ...
        const handleDelete = () => {
            if (window.confirm(`Are you sure you want to delete the page "${page.id}"?`)) { // Use page ID for confirmation
                deleteItem(page.id);
                    ...
                    {isEditing ? (
                        ...
                        <>
                            <button
                                ...
                                title="Edit Content"
                            >
                                <FaEdit size={20} /> {/* Using FaEdit as requested */}
                            </button>
                            <button
...
export default function AllPagesView() {
    ...
    useEffect(() => {
        ...
        if (pageEl) {
            pageEl.scrollIntoView({ behavior: 'smooth', block: 'center' }); // Added block: 'center'
        }
    }, [activePageId]);
